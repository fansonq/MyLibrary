package com.example.fansonlib.widget.stepprogress;
...
//        mLinePadding = 0.85f * defaultStepIndicatorNum;
        ...
        mDefaultIcon = ContextCompat.getDrawable(getContext(), R.mipmap.ic_not_complete);//未完成的icon
    }
...
//            float paddingLeft = (screenWidth - mStepNum * mCircleRadius * 2 - (mStepNum - 1) * mLinePadding) / 2;
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
            ...
            if (i <= mComplectingPosition && mStepBeanList.get(0).getState() != StepBean.STEP_CURRENT)//判断在完成之前的所有点
            {
