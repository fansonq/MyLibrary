...
public class BaseBuilder {
    ...
    public BaseBuilder setChannelId(String channelId) {
    ...
    public void build() {
        cBuilder = new NotificationCompat.Builder(MyNotificationUtils.mContext, mChannelId);
        cBuilder.setContentIntent(contentIntent);// 该通知要启动的Intent
        if (smallIcon > 0) {
            cBuilder.setSmallIcon(smallIcon);// 设置顶部状态栏的小图标
        }
        if (bigIcon > 0) {
            cBuilder.setLargeIcon(BitmapFactory.decodeResource(MyNotificationUtils.mContext.getResources(), bigIcon));
        }
        if (mChannelId != null) {
