...
/**
 * Created by：fanson
 * Created on：2016/12/17 11:32
 * Describe：
 */
public class MyProgressWheel extends View{

    private static final String TAG = MyProgressWheel.class.getSimpleName();
    ...
    public MyProgressWheel(Context context, AttributeSet attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.MyProgressWheel));
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius =
                (int) a.getDimension(R.styleable.MyProgressWheel_matProg_circleRadius, circleRadius);

        fillRadius = a.getBoolean(R.styleable.MyProgressWheel_matProg_fillRadius, false);

        barWidth = (int) a.getDimension(R.styleable.MyProgressWheel_matProg_barWidth, barWidth);

        rimWidth = (int) a.getDimension(R.styleable.MyProgressWheel_matProg_rimWidth, rimWidth);

        float baseSpinSpeed =
                a.getFloat(R.styleable.MyProgressWheel_matProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime =
                a.getInt(R.styleable.MyProgressWheel_matProg_barSpinCycleTime, (int) barSpinCycleTime);

        barColor = a.getColor(R.styleable.MyProgressWheel_matProg_barColor, barColor);

        rimColor = a.getColor(R.styleable.MyProgressWheel_matProg_rimColor, rimColor);

        linearProgress = a.getBoolean(R.styleable.MyProgressWheel_matProg_linearProgress, false);

        if (a.getBoolean(R.styleable.MyProgressWheel_matProg_progressIndeterminate, false)) {
            spin();
        }
